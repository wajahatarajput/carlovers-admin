...
const PaidCardsRequests: React.FC = () => {
  const { getPaidOrders, orders, getOrders } = useOrders();
  ...
  useEffect(() => {
    reloadData();
  }, [getPaidOrders, reloadData]);

  if (orders?.length > 0) {
    return (
      <AuthGuard requiredAbility={['manage', 'workshop-dashboard']}>
        <Layout>
          <div className="row d-xl-flex justify-content-xl-center pt-5 align-items-xl-center rounded .bg-secondary.bg-gradient" style={{ margin: "0px" }}>
...
export default PaidCardsRequests;
